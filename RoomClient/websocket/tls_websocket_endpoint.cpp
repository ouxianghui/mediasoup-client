...
#include "tls_websocket_endpoint.h"
...
namespace vi {
	TLSWebsocketEndpoint::TLSWebsocketEndpoint()
		: _nextId(0) {
        ...
        _thread = websocketpp::lib::make_shared<websocketpp::lib::thread>(&TLSClient::run, &_endpoint);
	}
    ...
    int TLSWebsocketEndpoint::connect(std::string const& uri, std::shared_ptr<IConnectionObserver> observer, const std::string& subprotocol) {
		websocketpp::lib::error_code ec;

        TLSClient::connection_ptr con = _endpoint.get_connection(uri, ec);
		...
		con->set_fail_handler(websocketpp::lib::bind(
			&ConnectionMetadata<TLSClient>::onFail,
			metadataPtr,
			&_endpoint,
			websocketpp::lib::placeholders::_1
		));
		con->set_close_handler(websocketpp::lib::bind(
			&ConnectionMetadata<TLSClient>::onClose,
			metadataPtr,
			&_endpoint,
			websocketpp::lib::placeholders::_1
		));
		con->set_message_handler(websocketpp::lib::bind(
			&ConnectionMetadata<TLSClient>::onMessage,
			metadataPtr,
			&_endpoint,
			websocketpp::lib::placeholders::_1,
		...
		con->set_ping_handler(websocketpp::lib::bind(
			&ConnectionMetadata<TLSClient>::onPing,
			metadataPtr,
			&_endpoint,
			websocketpp::lib::placeholders::_1,
			websocketpp::lib::placeholders::_2
		...
		con->set_pong_handler(websocketpp::lib::bind(
			&ConnectionMetadata<TLSClient>::onPong,
			metadataPtr,
			&_endpoint,
			websocketpp::lib::placeholders::_1,
			websocketpp::lib::placeholders::_2
		...
		con->set_pong_timeout_handler(websocketpp::lib::bind(
			&ConnectionMetadata<TLSClient>::onPongTimeout,
			metadataPtr,
			&_endpoint,
			websocketpp::lib::placeholders::_1,
			websocketpp::lib::placeholders::_2
	...
	void TLSWebsocketEndpoint::close(int id, websocketpp::close::status::value code, const std::string& reason) {
		websocketpp::lib::error_code ec;
	...
	void TLSWebsocketEndpoint::sendText(int id, const std::string& data) {
		websocketpp::lib::error_code ec;
	...
	void TLSWebsocketEndpoint::sendBinary(int id, const std::vector<uint8_t>& data)
	{
		websocketpp::lib::error_code ec;
	...
	void TLSWebsocketEndpoint::sendPing(int id, const std::string& data) {
		websocketpp::lib::error_code ec;
	...
	void TLSWebsocketEndpoint::sendPong(int id, const std::string& data) {
		websocketpp::lib::error_code ec;
	...
	ConnectionMetadata<TLSClient>::ptr TLSWebsocketEndpoint::getMetadata(int id) const {
		ConnectionList::const_iterator metadataIt = _connectionList.find(id);
		if (metadataIt == _connectionList.end()) {
			return ConnectionMetadata<TLSClient>::ptr();
		}
		else {
