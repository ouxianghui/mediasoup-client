...
void MacCapturer::start() {
    AVCaptureDevice *device = [[RTC_OBJC_TYPE(RTCCameraVideoCapturer) captureDevices] objectAtIndex:capture_device_index_];
    AVCaptureDeviceFormat *format = SelectClosestFormat(device, width_, height_);
    RTC_OBJC_TYPE(RTCCameraVideoCapturer) *capturer =  (__bridge_transfer RTC_OBJC_TYPE(RTCCameraVideoCapturer) *)capturer_;
    [capturer startCaptureWithDevice:device format:format fps:target_fps_];
}
