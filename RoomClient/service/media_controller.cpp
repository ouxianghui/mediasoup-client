...
//                _capturerSource = WindowsCapturerTrackSource::Create(_signalingThread);
    ...
    {
        DLOG("createNewConsumer: 0");
        _internalThread->PostTask([wself = weak_from_this(), request](){
            auto self = wself.lock();
            if (!self) {
                DLOG("RoomClient is null");
                return;
    ...
    void MediaController::onConsumerPaused(std::shared_ptr<signaling::ConsumerPausedNotification> notification)
    {
        ...
        _internalThread->PostTask([wself = weak_from_this(), tid](){
            auto self = wself.lock();
            if (!self) {
                DLOG("RoomClient is null");
                return;
    ...
    {
        _internalThread->PostTask([wself = weak_from_this(), notification](){
            auto self = wself.lock();
            if (!self) {
                DLOG("RoomClient is null");
                return;
